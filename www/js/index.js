...
//Botão configurar (Confgiuraçãoes)
	...
	//Inicio da contagemcada jogador
	 function start(){
		...
		if(delay==0){
			if(dvalor==2 |dvalor==3|dvalor==5 && lado==1){
			document.getElementById("delayPreto").innerText = "Delay: "+ 0;
			
			}
			if(dvalor==2 |dvalor==3|dvalor==5 && lado==0){
			...
			if (lado==0){
				if(dvalor==1){
					carregarDelay()
					document.getElementById("delayBranco").innerText = "Delay: "+ 1 ;
					delay=0;
			}else{
				...
				if(dvalor==1){
					carregarDelay()
					document.getElementById("delayPreto").innerText = "Delay: "+ 1 ;
					delay=0;
				}else{
